...
# The native handoff bridge (mTLS client certificate, HMAC, redeem URL) is retired (ADR-0048).
jq -e '
  ([.services[].environment // {} | keys[] | select(startswith("SKY_NATIVE_BRIDGE_"))] | length) == 0 and
  ([.services[].volumes // [] | .[] | select((.target // "") | startswith("/run/secrets/native-bridge"))] | length) == 0
' <<<"$rendered_json" >/dev/null \
  || fail 'the retired native handoff bridge must not come back into the production compose'
